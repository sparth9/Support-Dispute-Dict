...
public class Dictionary {
	...
	public static void main(String args[]) {
		...
		for(int s=0; s<3; s++){
			compare = calculate_synset(seed, compare,  type, s);
		}
		
		calculate_troponyms(compare);
		
		Synset[] synsets_hypernyms = database.getSynsets("believe", type);
...
/*		Connection con = null;
		try {
			...
			stmt.executeUpdate("INSERT INTO dictionary (seed, word, sim, type, form) VALUES ('"
					+ seed
					+ "', '"
					+ word
					+ "', '"
	...
	public static double cal_sim_1_hypernym(String word, String other, SynsetType type){
		...
		if( type.toString().equals("1") ){
				...
				for(int j=0; j<synsets_2.length; j++){
					...
					if(length != 0){
						//System.out.println("--------------------------------------------------------");
						System.out.println("LCA between("+word + "synset-"+(i+1) +" and "+ other +"synset-"+ (j+1) +"):"+length);
